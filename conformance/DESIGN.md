...
- Opaque leaves: meta-schema rejection when an opaque JSON object leaf declares no stance;
  ...
  for `consumer_check = "<name>"` and justified `unchecked`; the `dclrbl check` inventory of
  unchecked subtrees and consumer-check declarations (toolchain golden output).
...
- Versioning: gate fixtures (integer `format_version`, exact match, three messages + the
  ...
  diagnostics unchanged by streaming, per-line independence); manifest (dclrbl.toml)
  meta-fixtures incl. store/channel declarations; meta-migration fixtures (schema-language
  version bumps ship declarative meta-migrations; `dclrbl migrate` upgrades consumer schema
  files like any document; `dclrbl_spec_version` gating). Down-chains after up-chains are
  green.
...
- Toolchain (CLI): `dclrbl migrate` atomicity — all-or-nothing per run (transform+revalidate
  to temp, rename sweep only after all succeed; a chain that fails on file N leaves all files
  untouched, zero disk changes); `dclrbl migrate --dry-run` produces zero disk changes and a
  per-file structured diff; `dclrbl init` manifest skeleton + .gitattributes, hard error if a
  manifest exists.
- DIFF ENGINE (toolchain golden output; EMPIRICAL — the formal analyzer is unbundled,
  ...
  killing diagnostics) and migrate-round-trip outcomes (soundness/completeness
  ...
  adjudication-file validation (it is a dclrbl-schema'd document) and the gate-blocking
...
- The `dclrbl check` gate exercised here (stale code fails; pairing mismatch fails; the
  unchecked inventory renders); `dclrbl gen` hard-errors when a TS target is declared for a
...
Source: the hand-written dclrbl TOML translation of PixelWeaver's character-preview schema
...
- Verdict parity STRICT except a committed WAIVER LIST where divergence is the pass condition.
  ...
  pydantic lax int coercion) AND the expected dclrbl diagnostic codes — the entry asserts
  dclrbl's correct behavior, not merely "ignore this divergence." The lexical-number class
  wholesale on legacy targets is waived the same way (legacy TS necessarily parsed with
  JSON.parse; legacy pydantic coerces) — dclrbl's stricter verdicts on those fixtures are
  correct by definition. Once the acceptance test goes green the list is FROZEN: adding any
  entry afterward is a hard error.
- On reject: normalized legacy path SET must be a subset of dclrbl's (pydantic sprays per-arm
  errors and stops at the first model_validator; dclrbl reports one arm, all errors).

Timing: at MVP time, from the schema alone, BEFORE any consumer migration. Roadmap step 6 is
...
- tunebox: ~33 mirrored Py/Go failure cases -> shared fixtures.
- toolstream: ~26 type-mapping cases.
- wakethemup: 26 TestParse_* assertions -> fixture expectations (codes + paths).
- PixelWeaver: the acceptance corpus above.
